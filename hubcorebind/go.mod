module hubcore/hubcorebind

go 1.26.1

require (
	hubcore/rnsbind v0.0.0
	hubcore/yggbind v0.0.0
)

require (
	github.com/Arceliar/ironwood v0.0.0-20260117132459-7017dbc41d8e // indirect
	github.com/Arceliar/phony v0.0.0-20220903101357-530938a4b13d // indirect
	github.com/JuulLabs-OSS/cbgo v0.0.2 // indirect
	github.com/bits-and-blooms/bitset v1.24.4 // indirect
	github.com/bits-and-blooms/bloom/v3 v3.7.1 // indirect
	github.com/coder/websocket v1.8.14 // indirect
	github.com/creack/goselect v0.1.3 // indirect
	github.com/dsnet/compress v0.0.1 // indirect
	github.com/go-ble/ble v0.0.0-20240122180141-8c5522f54333 // indirect
	github.com/go-ole/go-ole v1.3.0 // indirect
	github.com/godbus/dbus/v5 v5.2.2 // indirect
	github.com/gologme/log v1.3.0 // indirect
	github.com/hjson/hjson-go/v4 v4.6.0 // indirect
	github.com/mattn/go-colorable v0.1.14 // indirect
	github.com/mattn/go-isatty v0.0.20 // indirect
	github.com/mgutz/ansi v0.0.0-20200706080929-d51e80ef957d // indirect
	github.com/mgutz/logxi v0.0.0-20161027140823-aebf8a7d67ab // indirect
	github.com/pkg/errors v0.9.1 // indirect
	github.com/quic-go/quic-go v0.59.0 // indirect
	github.com/raff/goble v0.0.0-20200327175727-d63360dcfd80 // indirect
	github.com/saltosystems/winrt-go v0.0.0-20260317170058-9c2fec580d96 // indirect
	github.com/sirupsen/logrus v1.9.4 // indirect
	github.com/soypat/cyw43439 v0.0.0-20260321224103-c7bb279d9c72 // indirect
	github.com/soypat/lneto v0.0.0-20260331220408-22c7e6c9d80b // indirect
	github.com/soypat/seqs v0.0.0-20260125140838-2c1c6b1bd69e // indirect
	github.com/svanichkin/configobj v0.0.1 // indirect
	github.com/svanichkin/go-reticulum v1.1.4 // indirect
	github.com/svanichkin/i2plib v0.0.1 // indirect
	github.com/tarm/serial v0.0.0-20180830185346-98f6abe2eb07 // indirect
	github.com/tinygo-org/cbgo v0.0.4 // indirect
	github.com/tinygo-org/pio v0.3.0 // indirect
	github.com/yggdrasil-network/yggdrasil-go v0.5.13 // indirect
	go.bug.st/serial v1.6.4 // indirect
	golang.org/x/crypto v0.49.0 // indirect
	golang.org/x/exp v0.0.0-20260312153236-7ab1446f8b90 // indirect
	golang.org/x/mobile v0.0.0-20260312152759-81488f6aeb60 // indirect
	golang.org/x/mod v0.34.0 // indirect
	golang.org/x/net v0.52.0 // indirect
	golang.org/x/sync v0.20.0 // indirect
	golang.org/x/sys v0.42.0 // indirect
	golang.org/x/text v0.35.0 // indirect
	golang.org/x/tools v0.43.0 // indirect
	tinygo.org/x/bluetooth v0.14.0 // indirect
)

replace (
	github.com/yggdrasil-network/yggdrasil-go => ../yggdrasil-go-develop
	hubcore/rnsbind => ../rnsbind
	hubcore/yggbind => ../yggbind
)
